        ...
        $rootNode
            ->children()
                ...
                ->arrayNode('export')
                    ...
                    ->end()
                ->end()
                ->arrayNode('hash_preserve')
                    ->info('Defaults for HashPreserveFaker (deterministic hashing with optional salt)')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->scalarNode('default_salt')
                            ->info('Salt applied when faker options omit salt. Null uses %%kernel.secret%%.')
                            ->defaultNull()
                        ->end()
                    ->end()
                ->end()
                ->arrayNode('environment_protection')
                    ...
                    ->children()
                        ->arrayNode('blocked_dsn_substrings')
                            ->info('Case-insensitive substrings matched against DATABASE_URL / MONGODB_URL, Doctrine connection url/host/dbname, and hosts')
                            ->prototype('scalar')->end()
                            ->defaultValue([
                                '://prod',
                                '@prod.',
