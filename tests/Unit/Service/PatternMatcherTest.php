...
class PatternMatcherTest extends TestCase
{
    ...
    /**
     * Test that list of pattern sets with a non-array element is not treated as list of sets (isListOfPatternSets returns false).
     */
    public function testIncludePatternsListWithNonArrayElementTreatedAsSingleSet(): void
    {
        $includePatterns = [
            ['id' => '>0'],
            'not_an_array',
    ...
    /**
     * Test pattern with pipe (OR): value matches one of the options (covers elseif str_contains/equals branch).
     */
    public function testMatchesWithPipeOption(): void
    {
        $record = ['status' => 'active'];
        $this->assertTrue($this->matcher->matches($record, ['status' => 'active|deleted']));
        $record = ['status' => 'deleted'];
        $this->assertTrue($this->matcher->matches($record, ['status' => 'active|deleted']));
        $record = ['status' => 'pending'];
        $this->assertFalse($this->matcher->matches($record, ['status' => 'active|deleted']));
    }
    ...
    /**
     * Test OR pattern (|) with options that match by contains or exact (no % in pattern so OR branch is used).
     */
    public function testMatchesWithOrPatternMultipleOptions(): void
    {
        $record = ['tag' => 'abmidcd'];
        $this->assertTrue($this->matcher->matches($record, ['tag' => 'nomatch|mid']));
        $this->assertTrue($this->matcher->matches($record, ['tag' => 'mid|other']));
        $this->assertFalse($this->matcher->matches($record, ['tag' => 'nomatch|nope']));
    }
    ...
    /**
     * Test OR pattern where no option matches (covers return false in patternMatchesValue).
     */
    ...
    /**
     * Test pattern match when value is numeric (hits final return $value == $pattern).
     */
    public function testMatchesWithNumericValueAndPattern(): void
    {
        $record = ['count' => 42];
        $this->assertTrue($this->matcher->matches($record, ['count' => '42']));
        $this->assertFalse($this->matcher->matches($record, ['count' => '43']));
    }

    /**
     * Test pattern match when value is not string (bool) and pattern is string (hits final return $value == $pattern).
     */
    public function testMatchesWithNonStringValueEqualsPattern(): void
    {
        $record = ['active' => true];
        $this->assertTrue($this->matcher->matches($record, ['active' => '1']));
        $this->assertTrue($this->matcher->matches($record, ['active' => 'true']));
        $record = ['flag' => false];
        $this->assertTrue($this->matcher->matches($record, ['flag' => '']));
        $this->assertFalse($this->matcher->matches($record, ['flag' => '1']));
    }

    /**
     * Test getNestedValue when field has dot and record has nested array (associationName => array).
     */
    public function testMatchesWithNestedAssociationArray(): void
    {
        $record = ['category' => ['name' => 'Premium']];
        $this->assertTrue($this->matcher->matches($record, ['category.name' => 'Premium']));
        $this->assertFalse($this->matcher->matches($record, ['category.name' => 'Standard']));
    }
    ...
    /**
     * Test <> operator (not equals alternative, checked before '<' so it is reachable).
     */
    public function testMatchesWithOperatorNotEqualsAlternate(): void
    {
        $record = ['status' => 'active'];
        $this->assertTrue($this->matcher->matches($record, ['status' => '<>deleted']));
        $this->assertFalse($this->matcher->matches($record, ['status' => '<>active']));

        $record2 = ['count' => 10];
        $this->assertTrue($this->matcher->matches($record2, ['count' => '<>5']));
        $this->assertFalse($this->matcher->matches($record2, ['count' => '<>10']));
    }
    ...
    /**
     * Test OR pattern where no option matches (covers return false after iterating options).
     */
    ...
    /**
     * Test nested field when key is missing in nested array (getNestedValue returns null from ?? null).
     */
    public function testMatchesWithNestedArrayMissingKeyReturnsNull(): void
    {
        $record = ['type' => ['name' => 'HR']];
        $this->assertFalse($this->matcher->matches($record, ['type.code' => 'HR001']));
    }
    ...
    /**
     * Test getNestedValue when association is present as array but relatedField key is missing (covers ?? null branch).
     */
    public function testMatchesWithNestedArrayPresentButRelatedFieldKeyMissing(): void
    {
        $record = ['type' => ['code' => 'X']];
        $this->assertFalse($this->matcher->matches($record, ['type.name' => 'HR']));
    }
