...
class AnonymizeServiceTest extends TestCase
{
    ...
    public function testGetAnonymizableEntitiesReturnsEntitiesWithAttribute(): void
    {
        ...
        $className = 'Nowo\AnonymizeBundle\Tests\Unit\Service\TestEntity';
    ...
    public function testGetAnonymizablePropertiesReturnsPropertiesWithAttribute(): void
    {
        ...
        $propertyNames = array_map(static fn (array $item): string => $item['property']->getName(), $result);
    ...
    public function testAnonymizeEntityHandlesProgressCallback(): void
    {
        ...
        $progressCallback = static function ($current, $total, $message) use (&$progressCalled): void {
            $progressCalled = true;
        };
    ...
    public function testAnonymizeEntityHandlesNonExistentProperties(): void
    {
        ...
        $progressCallback = static function ($current, $total, $message): void {
            // Empty callback
        };
        $result = $this->service->anonymizeEntity($em, $metadata, $reflection, $properties, 100, false, null, $progressCallback);
    ...
    /**
     * Test that anonymizeEntity returns propertyStats when a property is anonymized (covers lines 603-606, 660, 669).
     */
    public function testAnonymizeEntityReturnsPropertyStatsWhenPropertyUpdated(): void
    {
        ...
        $callCount = 0;
        $connection->method('fetchAllAssociative')
            ->willReturnCallback(static function () use ($record, &$callCount): array {
                ++$callCount;
        ...
        $connection->method('getDatabasePlatform')->willReturn($platform);
        $connection->method('quote')
            ->willReturnCallback(static fn ($val): string => "'" . str_replace("'", "''", (string) $val) . "'");
        ...
        $result = $this->service->anonymizeEntity($em, $metadata, $reflection, $properties, 100, false);

        $this->assertIsArray($result);
        $this->assertEquals(1, $result['processed']);
        $this->assertEquals(1, $result['updated']);
        $this->assertArrayHasKey('propertyStats', $result);
        $this->assertArrayHasKey('email', $result['propertyStats']);
        $this->assertSame(1, $result['propertyStats']['email']);
    }
    ...
    public function testAnonymizeEntityAddsDiscriminatorToQueryWhenPolymorphic(): void
    {
        ...
        $capturedQuery = null;
        $connection->method('fetchAllAssociative')
            ->willReturnCallback(static function ($query) use (&$capturedQuery): array {
                $capturedQuery = $query;
    ...
    public function testAnonymizeEntityHandlesNullValues(): void
    {
        ...
        $connection->method('getDatabasePlatform')->willReturn($platform);
        $connection->method('quote')
            ->willReturnCallback(static function (?string $val): string {
                if ($val === null) {
                    return 'NULL';
                }
    ...
    public function testAnonymizeEntityHandlesNullableOption(): void
    {
        ...
        $capturedSql = '';
        $connection->method('executeStatement')
            ->willReturnCallback(static function ($sql) use (&$nullUsed, &$capturedSql): int {
                $capturedSql = $sql;
    ...
    public function testAnonymizeEntityHandlesNullableOptionZeroProbability(): void
    {
        ...
        $nullUsed = false;
        $connection->method('executeStatement')
            ->willReturnCallback(static function ($sql) use (&$nullUsed): int {
    ...
    public function testAnonymizeEntityPreservesNullValues(): void
    {
        ...
        $sqlExecuted = false;
        $connection->method('executeStatement')
            ->willReturnCallback(static function ($sql) use (&$sqlExecuted): int {
                $sqlExecuted = true;
    ...
    public function testAnonymizeEntityBypassesEntityExclusion(): void
    {
        ...
        $capturedSql = '';
        $connection->method('executeStatement')
            ->willReturnCallback(static function ($sql) use (&$capturedSql): int {
                $capturedSql = $sql;
    ...
    public function testTruncateTablesTruncatesForMySQL(): void
    {
        ...
        $executedStatements = [];
        $connection->method('executeStatement')
            ->willReturnCallback(static function ($sql) use (&$executedStatements): int {
    ...
    public function testTruncateTablesOrdersByTruncateOrder(): void
    {
        ...
        $truncateStatements = [];
        $connection->method('executeStatement')
            ->willReturnCallback(static function ($sql) use (&$truncateStatements): int {
    ...
    /**
     * Test that truncateTables sorts by table name when two entities have the same truncate_order.
     */
    public function testTruncateTablesSortsByTableNameWhenSameOrder(): void
    {
        ...
        $truncateOrder = [];
        $connection->method('executeStatement')
            ->willReturnCallback(static function ($sql) use (&$truncateOrder): int {
                if (str_contains($sql, 'TRUNCATE TABLE')) {
                    $truncateOrder[] = $sql;
                }
        ...
        $entities = [
            'EntityB' => [
                'metadata'   => $metadataB,
                'reflection' => $this->createMock(ReflectionClass::class),
                'attribute'  => new Anonymize(truncate: true, truncate_order: 5),
            ],
            'EntityA' => [
                'metadata'   => $metadataA,
                'reflection' => $this->createMock(ReflectionClass::class),
        ...
        $this->assertIsArray($result);
        $this->assertCount(2, $result);
        $this->assertCount(2, $truncateOrder);
        $this->assertStringContainsString('`table_alpha`', $truncateOrder[0]);
        $this->assertStringContainsString('`table_beta`', $truncateOrder[1]);
    }
    ...
    public function testTruncateTablesHandlesSQLite(): void
    {
        ...
        $this->assertIsArray($result);
        $this->assertArrayHasKey('test_table', $result);
        // SQLite should use DELETE FROM, sqlite_sequence reset, and PRAGMA
        ...
        $this->assertTrue(
            array_reduce(
                $executedStatements,
                static fn ($carry, $s): bool => $carry || str_contains((string) $s, 'sqlite_sequence'),
                false,
            ),
    ...
    /**
     * Test that truncateTables uses DELETE FROM when driver is not MySQL, PostgreSQL or SQLite.
     */
    public function testTruncateTablesHandlesUnknownDriverWithDeleteFrom(): void
    {
        ...
        $this->assertSame(0, $result['custom_table']);
        $this->assertContains('DELETE FROM "custom_table"', $executedStatements);
    }
    ...
    public function testTruncateTablesUsesDeleteByDiscriminatorWhenPolymorphic(): void
    {
        ...
        $deleteSql = array_values(array_filter($executedStatements, static fn ($s): bool => str_contains((string) $s, 'DELETE FROM')));
    ...
    /**
     * Test that truncateTables calls progress callback with discriminator message when entity is polymorphic (covers line 232).
     */
    public function testTruncateTablesPolymorphicCallsProgressCallbackWithDiscriminatorMessage(): void
    {
        ...
        $progressMessages = [];
        $progressCallback = static function (string $table, string $msg) use (&$progressMessages): void {
            $progressMessages[] = ['table' => $table, 'msg' => $msg];
        };
        ...
        $this->assertStringContainsString('discriminator', $progressMessages[0]['msg']);
