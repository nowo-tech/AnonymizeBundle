...
class EmailFakerTest extends TestCase
{
    ...
    /**
     * Test that EmailFaker uses default branch for unknown format.
     */
    public function testGenerateWithDefaultFormatBranch(): void
    {
        $faker = new EmailFaker('en_US');
        $email = $faker->generate(['format' => 'custom']);
    ...
    /**
     * Test that EmailFaker explicitly uses 'random' format branch.
     */
    public function testGenerateWithFormatRandom(): void
    {
        $faker = new EmailFaker('en_US');
        $email = $faker->generate(['format' => 'random']);
    ...
    public function testGeneratePadsShortLocalPartWithNumbers(): void
    {
        ...
        for ($i = 0; $i < 20; ++$i) {
            ...
            $parts = explode('@', $email);
            $this->assertCount(2, $parts);
            $this->assertLessThanOrEqual(12, strlen($parts[0]));
            $this->assertGreaterThanOrEqual(1, strlen($parts[0]));
        }
    }
}
