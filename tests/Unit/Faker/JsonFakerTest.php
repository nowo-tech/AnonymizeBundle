...
class JsonFakerTest extends TestCase
{
    ...
    /**
     * Test that JsonFaker schema with unknown type uses default branch in generateValueByType.
     */
    public function testGenerateWithSchemaUnknownType(): void
    {
        $faker  = new JsonFaker('en_US');
        $schema = [
        ...
        $data = json_decode($json, true);
        $this->assertIsArray($data);
        $this->assertArrayHasKey('unknown_field', $data);
        $this->assertIsString($data['unknown_field']);
    }

    /**
     * Test that JsonFaker schema with type 'float' uses float branch in generateValueByType.
     */
    public function testGenerateWithSchemaFloatType(): void
    {
        $faker  = new JsonFaker('en_US');
        $schema = [
            'score' => ['type' => 'float'],
    ...
    /**
     * Test that JsonFaker schema with type 'array' uses array branch in generateValueByType.
     */
    public function testGenerateWithSchemaArrayType(): void
    {
        $faker  = new JsonFaker('en_US');
        $schema = [
            'tags' => ['type' => 'array'],
    ...
    /**
     * Test that JsonFaker schema with type 'object' and depth 0 returns empty array in generateValueByType.
     */
    public function testGenerateWithSchemaObjectTypeAndZeroDepth(): void
    {
        $faker  = new JsonFaker('en_US');
        $schema = [
            'nested' => ['type' => 'object'],
        ...
        $this->assertEmpty($data['nested']);
    }

    /**
     * Test that JsonFaker schema with type 'object' and depth > 0 returns nested structure (generateValueByType object branch).
     */
    public function testGenerateWithSchemaObjectTypeAndPositiveDepth(): void
    {
        $faker  = new JsonFaker('en_US');
        $schema = [
            'nested' => ['type' => 'object'],
        ...
        $this->assertArrayHasKey('nested', $data);
        $this->assertIsArray($data['nested']);
        // With depth 2, generateValueByType('object', 1) returns generateRandomStructure(1, 3), so non-empty
        $this->assertNotEmpty($data['nested']);
    }

    /**
     * Test that generateRandomStructure can produce boolean values (covers 'boolean' branch).
     */
    public function testGenerateRandomStructureCanProduceBoolean(): void
    {
        $faker = new JsonFaker('en_US');
        $found = false;
        for ($i = 0; $i < 80; ++$i) {
            $json = $faker->generate(['depth' => 2, 'max_items' => 5]);
            $data = json_decode($json, true);
            if ($data !== null && $this->arrayContainsBoolean($data)) {
                $found = true;
                break;
            }
        }
        $this->assertTrue($found, 'generateRandomStructure should eventually produce a boolean value');
    }

    /**
     * @param array<string, mixed> $arr
     */
    private function arrayContainsBoolean(array $arr): bool
    {
        foreach ($arr as $v) {
            if (is_bool($v)) {
                return true;
