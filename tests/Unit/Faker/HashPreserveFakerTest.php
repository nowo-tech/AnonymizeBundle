...
class HashPreserveFakerTest extends TestCase
{
    ...
    /**
     * Test that HashPreserveFaker sets hash to '0' when preserve_format yields no digits (branch coverage).
     * With length=1 we take the first hex char; when it is a-f, preg_replace leaves empty and we get '0'.
     */
    public function testGenerateWithPreserveFormatEmptyHashBranch(): void
    {
        $faker = new HashPreserveFaker();
        $found = false;
        for ($i = 0; $i < 500; ++$i) {
            $hash = $faker->generate(['value' => (string) $i, 'preserve_format' => true, 'length' => 1]);
            if ($hash === '0') {
                $found = true;
                break;
            }
        }
        $this->assertTrue($found, 'Expected at least one numeric value to produce hash with first char a-f (empty after preg_replace), so result is "0".');
    }
