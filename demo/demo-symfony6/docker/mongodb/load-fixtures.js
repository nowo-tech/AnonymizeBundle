...
for (let i = 1; i <= 20; i++) {
    ...
    transactions.push({
        ...
        transactionHash: require('crypto').createHash('sha256').update(`transaction${i}`).digest('hex'),
...
for (let i = 1; i <= 15; i++) {
    ...
    devices.push({
        ...
        deviceHash: require('crypto').createHash('sha256').update(`device${i}`).digest('hex'),
...
for (let i = 1; i <= 35; i++) {
    ...
    events.push({
        ...
        userIdHash: require('crypto').createHash('sha256').update(`user${i}`).digest('hex'),
