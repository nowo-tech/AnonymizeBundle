...
- PHP (Lines): **100%** (see `make test-coverage` for the PHPUnit summary and HTML report)
- TS/JS: N/A
- Python: N/A
...
- **Test Coverage**: Large PHPUnit suite (**1100+** test methods; exact count changes as tests are added). Run `composer test` or `make test` for the current total. **100%** line coverage on `src/` (PHPUnit Lines) — see `make test-coverage` for the report.
