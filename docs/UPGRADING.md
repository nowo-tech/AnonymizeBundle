...
- **Maintenance**: demo Symfony 8 lockfile / config reference refresh (Symfony 8.1.2); root tooling lock bumps (php-cs-fixer, rector). No runtime API changes.
...
- **Maintainer tooling (REQ-MAKE-010)**: root and aggregate `demo/` Makefiles use Compose V2 (`docker compose`) with V1 fallback, consistent with `demo/symfony8`.
...
None for bundle consumers. If you relied on the **Symfony 6 demo** in this repository, use `demo/symfony7` or `demo/symfony8` instead.
