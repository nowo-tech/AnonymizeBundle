...
- The bundle resolves the service by id (FQCN or alias) via the container. **The service must be declared as `public: true`** in your `config/services.yaml` (or equivalent), otherwise the container may inline it at compile time and you will get "service has been removed or inlined". See the demos: `SmsNotification` and `SmsNotificationAnonymizerService` in `demo/symfony6`, `demo/symfony7`, and `demo/symfony8` (e.g. `App\Service\SmsNotificationAnonymizerService` with `public: true`).
