...
2. **Product behavior** — what **AnonymizeBundle** guarantees to applications that integrate it (see [`USAGE.md`](USAGE.md), [`CONFIGURATION.md`](CONFIGURATION.md), [`INSTALLATION.md`](INSTALLATION.md)). **PHPUnit** and **PHPStan** (and **Vitest** when applicable) enforce contracts in CI where applicable.
...
| ID | Story |
| --- | --- |
| US-01 | **As a** developer, **I want** Doctrine attributes on entities **so that** anonymization rules live beside the schema. |
| US-02 | **As a** developer, **I want** pluggable Faker types **so that** emails, IBANs, and masks replace PII realistically in dev/test. |
| US-03 | **As an** operator, **I want** CLI batch runs with dry-run and history **so that** I audit what changed before sharing databases. |
| US-04 | **As a** security officer, **I want** production execution blocked **so that** anonymize never runs against prod by mistake. |
| US-05 | **As a** maintainer, **I want** PHPUnit and PHPStan in CI **so that** Faker and batch logic regressions are caught early. |
...
**Goal:** Symfony bundle for anonymizing database records using Doctrine attributes and Faker generators. Perfect for test data generation, GDPR compliance, and data masking in development environments.
